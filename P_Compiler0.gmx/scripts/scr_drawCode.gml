...
draw_text_ext(10, 10, _txt, 20, rw/3);

//sombreado
draw_set_color(c_yellow);
for(var i = 0; i < array_length_1d(cCode); i++){
    if (executed[i]){
        draw_rectangle(floor(rw/3)+10, 10+string_height("L")*(i+1), floor(rw/3)+10+string_width(cCode[i]), 10+string_height("L")*(i+2)-2, 0); 
    }
    if (i = lastLineExed){
        draw_set_color(c_red);
        draw_rectangle(floor(rw/3)+10, 10+string_height("L")*(i+1), floor(rw/3)+10+string_width(cCode[i]), 10+string_height("L")*(i+2)-2, 0); 
...
draw_text_ext(floor(rw/3)+10, 10, _txt, 20, rw/3);
...
draw_set_color(c_red);
draw_text_ext(floor(rw/3)-15, 10, _txt, 20, rw/3);
...
draw_text_ext(floor(rw/3)*2+10, 10, _txt, 20, rw/3);
